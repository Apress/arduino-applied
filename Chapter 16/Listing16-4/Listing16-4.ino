...
void loop()
{
  ...
  if(flag == 1)       // if control sequence read
  {
    ...
    {
      case 10:            // turn LED 3 or 4 on or off
      if(val[1] == 3 || val[1] == 4) digitalWrite(val[1],!digitalRead(val[1]));
      else if(val[1] == 5)
      {
        for (int i=0;i<10;i++)
        {           // turn greenLED on and off
          digitalWrite(greenLED, !digitalRead(greenLED)); //    five times
